...
check_if_run_with_pi() {
  ...
  if [ $(id -ur) -ne $(id -ur pi) ]; then
    echo "DI-Sensors installer script must be run with \"pi\" user. Exiting."
    exit 7
  fi
}
...
parse_cmdline_arguments() {
  ...
  if [ -f $DEXTERSCRIPT/functions_library.sh ]; then
    ...
    # show some feedback for the DI_Sensors
    if [[ quiet_mode -eq 0 ]]; then
      ...
      echo " |  __ \_   _|    / ____|                           ";
      echo " | |  | || |_____| (___   ___ _ __  ___  ___  _ __ ___ ";
      echo " | |  | || |______\___ \ / _ \ '_ \/ __|/ _ \| '__/ __|";
      echo " | |__| || |_     ____) |  __/ | | \__ \ (_) | |  \__ \ ";
      echo " |_____/_____|   |_____/ \___|_| |_|___/\___/|_|  |___/";
    fi

    feedback "Welcome to DI_Sensors Installer."
  else
    echo "Welcome to DI_Sensors Installer."
  fi

  echo "Updating DI_Sensors for $selectedbranch branch with the following options:"
...
#################################################
# Cloning DI_Sensors, Script_Tools & RFR_Tools ##
#################################################
...
clone_disensors() {
  ...
  git clone --quiet --depth=1 -b feature/use-rfr-tools-too https://github.com/RobertLucian/DI_Sensors.git
  cd $SENSOR_DIR
}
