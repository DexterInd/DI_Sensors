...
parse_cmdline_arguments() {
  ...
  if [ -f $DEXTERSCRIPT/functions_library.sh ]; then
    ...
    # show some feedback for the DI-Sensors
    if [ ! quiet_mode ]; then
      echo "  _____            _                                ";
      echo " |  __ \          | |                               ";
    ...
    feedback "Welcome to DI-Sensors Installer."
  else
    echo "Welcome to DI-Sensors Installer."
  fi
  echo "Updating DI-Sensors for $selectedbranch branch with the following options:"
...
####################################################
######## Cloning DI-Sensors & Script_Tools  ########
####################################################

# called way down below
clone_disensors_and_scriptools() {
